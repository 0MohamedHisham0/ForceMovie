...
public class TopPopular_Frag extends Fragment {
    //Views
    View view;
    ObservableRecyclerView RV_TopPopular;
    RecyclerView RV_PagesNumber;
    SpinKitView progress_circular_TopPopular;
    Button Search;
    //Var
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.frag_popular, container, false);
        return view;
    }
    ...
    private void initViews() {
        ...
        RV_PagesNumber = view.findViewById(R.id.RV_TopPopular_Pages);
        RV_TopPopular = view.findViewById(R.id.RV_TopPopular);
        progress_circular_TopPopular = view.findViewById(R.id.progress_circular_TopPopular);
        Search = view.findViewById(R.id.Btn_Search_Popular);
    ...
    public void initAdapter(List<MoviesLists_Model.MovieDetail> list) {
        ...
        RV_TopPopular.setAdapter(adapterTopMovies);
        RecyclerView.LayoutManager layoutManager = new GridLayoutManager(getContext(), 2);

        RV_TopPopular.setScrollViewCallbacks(new ScrollOB(RV_PagesNumber));
        RV_TopPopular.setLayoutManager(layoutManager);

    }

    public void initRV_Pages() {
        LinearLayoutManager layoutManager = new LinearLayoutManager(getContext(), LinearLayoutManager.HORIZONTAL, true);
        RV_PagesNumber.setLayoutManager(layoutManager);
        new ListCounterAsy(RV_PagesNumber, getContext(), TopPopular_Frag.this, "").execute(Integer.parseInt(TotalPage));

    }

    //initializing Data
    public void TopMoviesData(String page_num) {
        progress_circular_TopPopular.setIndeterminateDrawable(progress_style);
        progress_circular_TopPopular.setVisibility(View.VISIBLE);
        RV_TopPopular.setVisibility(View.GONE);
        Call<MoviesLists_Model> call = RetrofitClient.getInstance().GetPopularMovies(api_key, "en-US", page_num);
            ...
            public void onResponse(Call<MoviesLists_Model> call, Response<MoviesLists_Model> response) {
                if (response.isSuccessful() && response.code() == 200) {
                    ...
                    handler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            RV_TopPopular.setVisibility(View.VISIBLE);
                            progress_circular_TopPopular.setVisibility(View.GONE);
                        }
                    }, 50);
